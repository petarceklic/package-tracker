...
const CARRIERS = {
  ...
  'Amazon': {
    ...
    trackingUrl: 'https://www.amazon.com.au/progress-tracker/package/ref=ppx_yo_dt_b_track_package?_encoding=UTF8&itemId=&orderId=&packageIndex=&shipmentId='
  },
  'AliExpress': {
...
function extractTrackingInfo(emailBody, emailSubject, fromEmail) {
  // Determine carrier from email domain or subject
...
function scanGmail() {
  console.log('📧 Scanning Gmail for shipping notifications...');
  
  const daysBack = process.env.SCAN_DAYS_BACK || 90;
  const searchQuery = `newer_than:${daysBack}d (tracking OR delivery OR shipped OR shipment OR "out for delivery" OR "package" OR "parcel")`;
  
  let result;
  try {
    result = execSync(
      `gog gmail search '${searchQuery}' --max 200 --json`,
  ...
  for (const thread of threads) {
    try {
      ...
      const info = extractTrackingInfo(body, subject, from);
      
      if (info.carrier && info.trackingNumber) {
        // Extract item description from subject
  ...
  console.log(`\n🎉 Scan complete! Found ${packagesFound} new packages`);
